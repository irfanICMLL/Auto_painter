...
parser.add_argument("--output_dir", default='wganups/train_auto_2', help="where to put output files")
parser.add_argument("--seed",type=int)
parser.add_argument("--checkpoint",default='wganups/train_auto',help="directory with checkpoint to resume training from or use for testing")
...
parser.add_argument("--tv_weight", type=float, default=0.001, help="weight on tv term for generator gradient")
parser.add_argument("--f_weight", type=float, default=0.0001, help="weight on f term for generator gradient")
...
def create_model(inputs, targets, net1,net2):
    ...
    with tf.name_scope("generator_loss"):
        ...
        gen_loss_tv = tf.reduce_mean(tf.abs(sum_tv_loss(outputs)))
        #out = tf.image.resize_images(outputs, [224, 224], method=tf.image.ResizeMethod.BICUBIC)
        #tar = tf.image.resize_images(targets, [224, 224], method=tf.image.ResizeMethod.BICUBIC)
        #net.build(out)
       # f_out = net.conv4_3
       # net.build(tar)
       # f_tar = net.conv4_3
