...
parser.add_argument("--input_dir", default='yelloman_self_train', help="path to folder containing images")
parser.add_argument("--mode", default='train', choices=["train", "test", "export"])
parser.add_argument("--output_dir", default='wgan-gp/yellowman', help="where to put output files")
...
parser.add_argument("--l1_weight", type=float, default=0.001, help="weight on L1 term for generator gradient")
parser.add_argument("--gan_weight", type=float, default=3, help="weight on GAN term for generator gradient")
parser.add_argument("--tv_weight", type=float, default=1e-3, help="weight on tv term for generator gradient")
parser.add_argument("--f_weight", type=float, default=1e-7, help="weight on f term for generator gradient")
...
def create_model(inputs, targets, net1,net2):
    ...
    def sum_tv_loss(image):
        loss_y = tf.sqrt(tf.nn.l2_loss(image[:, 1:, :, :] - image[:, :-1, :, :]))
        loss_x = tf.sqrt(tf.nn.l2_loss(image[:, :, 1:, :] - image[:, :, :-1, :]))
    ...
    with tf.name_scope("discriminator_loss"):
        ...
        with tf.variable_scope("discriminator", reuse=True):
          gradients = tf.gradients(create_discriminator(inputs, interpolates), [interpolates])[0]
        slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), reduction_indices=[1]))
        gradient_penalty = tf.reduce_mean((slopes-1.)**2)
        discrim_loss += 10*gradient_penalty
        #  with tf.name_scope(""):
    with tf.name_scope("generator_loss"):
        # predict_fake => 1
        # abs(targets - outputs) => 0
        gen_loss_GAN = -tf.reduce_mean(predict_fake)
        gen_loss_L1 = tf.sqrt(tf.nn.l2_loss(targets - outputs))
        gen_loss_tv = sum_tv_loss(outputs)
        gen_loss_f = tf.sqrt(tf.nn.l2_loss(feature_loss(targets,net1) - feature_loss(outputs,net2)))
    ...
    with tf.name_scope("discriminator_train"):
        ...
        discrim_optim =  tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, beta2=0.9)
    ...
    with tf.name_scope("generator_train"):
        with tf.control_dependencies([discrim_train]):
            ...
            gen_optim = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, beta2=0.9)
