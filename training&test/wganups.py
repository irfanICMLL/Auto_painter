...
parser.add_argument("--input_dir", default='/home/bo718.wang/irfan/3match_color/', help="path to folder containing images")
parser.add_argument("--mode", default='test', choices=["train", "test", "export"])
parser.add_argument("--output_dir", default='wganups/wgan_3match_test', help="where to put output files")
parser.add_argument("--seed", type=int)
parser.add_argument("--checkpoint", default='wganups/wgan_3match',help="directory with checkpoint to resume training from or use for testing")
...
parser.add_argument("--ndf", type=int, default=48, help="number of discriminator filters in first conv layer")
...
parser.add_argument("--gan_weight", type=float, default=2, help="weight on GAN term for generator gradient")
parser.add_argument("--tv_weight", type=float, default=1e-3, help="weight on tv term for generator gradient")
parser.add_argument("--f_weight", type=float, default=1e-5, help="weight on f term for generator gradient")
...
def deconvz(data, num_filter, kernel, pad):
    with tf.variable_scope("deconv"):
        data = tf.image.resize_bilinear(data, [int(x) for x in [data.shape[1]*2, data.shape[2]*2]])
        data = tf.layers.conv2d(data, num_filter, (kernel,kernel), (1,1), 'same')
      #  data = layers.layer_norm(data, scale=False)
        data = tf.nn.relu(data)
        return data

def deconvzz(data, num_filter, kernel, pad):
    with tf.variable_scope("deconv"):
        data = tf.image.resize_bilinear(data, [int(x) for x in [data.shape[1]*2, data.shape[2]*2]])
        data = tf.layers.conv2d(data, num_filter, (kernel,kernel), (1,1), 'same')
...
def load_examples():
  ...
  #  input_paths = os.listdir(sketch_dir)
...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    for decoder_layer, (out_channels, dropout) in enumerate(layer_specs):
        ...
        with tf.variable_scope("decoder_%d" % (skip_layer + 1)):
           ...
           # rectified = tf.nn.relu(input)
            # [batch, in_height, in_width, in_channels] => [batch, in_height*2, in_width*2, out_channels]
            output = deconvz(input, out_channels, 3, 1)
    ...
    with tf.variable_scope("decoder_1"):
        ...
        output = deconvzz(input, generator_outputs_channels, 3, 1)
...
def create_model(inputs, targets, net1,net2):
    ...
    with tf.name_scope("discriminator_train"):
        ...
        clip_vars = [tf.assign(var, tf.clip_by_value(var, -0.5, 0.5)) for var in discrim_tvars]
